...
function generateWarrior(id: string, style: FightingStyle) {
...
async function main() {
  console.log("Starting long-term headless simulation for Balance & Meta Report...");
  ...
  for (let i = 0; i < 20; i++) {
    state.roster.push(generateWarrior(`player-${i}`, styles[i % styles.length]));
  }
  ...
  for (let r = 0; r < 3; r++) {
    const roster = [];
    for (let i = 0; i < 20; i++) {
      roster.push(generateWarrior(`rival-${r}-${i}`, styles[(i + r) % styles.length]));
    }
    state.rivals.push({
  ...
  for (let i = 0; i < WEEKS_TO_SIM; i++) {
    ...
    while (state.roster.filter(w => w.status === "Active").length < 10) {
       state.roster.push(generateWarrior(`player-rep-${i}-${Math.random()}`, styles[Math.floor(Math.random() * styles.length)]));
    }
    for (const rival of state.rivals) {
       while (rival.roster.filter(w => w.status === "Active").length < 10) {
         rival.roster.push(generateWarrior(`rival-rep-${i}-${Math.random()}`, styles[Math.floor(Math.random() * styles.length)]));
       }
    }
