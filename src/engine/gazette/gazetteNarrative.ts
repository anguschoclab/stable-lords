 ...
 */
export function generateFightNarrative(
  fight: FightSummary,
  mood: CrowdMoodType,
  rng?: IRNGService
): string {
...
export function generateGazetteHeadline(
  ...
  if (detections.hotStreakers.length > 0) {
    const top = detections.hotStreakers.reduce(
      (max, curr) => (curr.streak > max.streak ? curr : max),
      detections.hotStreakers[0] as { name: string; streak: number }
    );
    if (!top) {
    ...
    const rivalryHeadline =
      detections.rivalryPair.count >= 5 ? gh.LegacyRivalry || gh.Rivalry : gh.Rivalry || gh.Rivalry;
    return t(rngService.pick(rivalryHeadline), {
      week,
      a: detections.rivalryPair.a,
      b: detections.rivalryPair.b,
      count: detections.rivalryPair.count,
    });
  } else if (detections.risingStars.length > 0) {
    return t(rngService.pick(gh.RisingStar || gh.Standard), {
      week,
      name: detections.risingStars[0],
    });
...
export function generateGazetteBody(
  ...
  if (detections.rivalryPair) {
    paragraphs.push(
      t(rngService.pick(detections.rivalryPair.count >= 5 ? gf.LegacyRivalry : gf.Rivalry), {
        a: detections.rivalryPair.a,
        b: detections.rivalryPair.b,
        count: detections.rivalryPair.count,
        suffix: detections.rivalryPair.count === 3 ? 'rd' : 'th',
      })
    );
  }
...
export function generateSeasonSummary(
  ...
  const body = [
    ...
    topStyle && gs.body[1]
      ? t(gs.body[1], { style: styleName(topStyle[0]), wins: topStyle[1] })
      : '',
