...
export const WorldManagementService = {
   ...
   */
  processSeasonalChurn(state: GameState, rngContext: IRNGContext): { updatedRivals: RivalStableData[]; news: string[] } {
    let updatedState = { ...state };
    const news: string[] = [];

    // Retirement
    const rng = rngContext.getRNG();
    const { updatedState: retirementState, legacyCandidates } = SeasonalRetirementService.processSeasonalRetirement(updatedState, rng);
    ...
    // Bankruptcy
    const { updatedState: bankruptcyState, bankruptStables } = BankruptcyService.processBankruptcy(updatedState, rng);
    ...
    const { updatedState: expansionState, newStables } = ExpansionService.processExpansion(updatedState, expansionRng, 45, this.legacyCandidates);
    updatedState = expansionState;
    
    newStables.forEach(ns => {
      const isLegacy = this.legacyCandidates.some(lc => lc.name === ns.owner.name);
      if (!isLegacy) {
        news.push(`🆕 RECRUITMENT: ${ns.owner.stableName} has been granted an arena license as a new Minor rival!`);
      }
    });
