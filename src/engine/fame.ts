...
export function fameFromTags(tags: string[] = []): FamePop {
  ...
  if (has("bloody")) { pop += 2; labels.push("Popularity +2 (Bloody)"); }
  if (has("upset")) { pop += 5; labels.push("Popularity +5 (Upset)"); }
  if (has("quick")) { pop -= 1; labels.push("Popularity -1 (Quick)"); }
  if (has("epic")) { pop += 10; labels.push("Popularity +10 (Epic)"); }
