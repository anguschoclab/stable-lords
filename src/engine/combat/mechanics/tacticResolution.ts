...
export function getOffensiveTacticMods(tactic: OffensiveTactic | undefined, style: FightingStyle) {
  ...
  switch (tactic) {
    ...
    case "Slash":        return { attBonus: 0, dmgBonus: Math.round(2 * mult), defPenalty: 0, endCost: 1, decBonus: 0, parryBypass: Math.round(2 * mult) };
...
export function calculateFinalOEAL(effOE: number, effAL: number, plan: FightPlan, hp: number, maxHp: number, end: number, maxEnd: number, exchange: number): [number, number] {
  let openOE = 0, openAL = 0;
  if (exchange < 3) {
    if (plan.openingMove === "Aggressive") { openOE = 1; openAL = 1; }
    else if (plan.openingMove === "Safe") { openOE = -1; openAL = -1; }
  }

  let fallOE = 0, fallAL = 0;
  if (plan.fallbackCondition === "FLEE" && hp < maxHp * 0.3) { fallOE = -3; fallAL = -3; }
  else if (plan.fallbackCondition === "TURTLE" && end < maxEnd * 0.3) { fallOE = -4; fallAL = 2; }
  else if (plan.fallbackCondition === "BERZERK" && hp < maxHp * 0.3) { fallOE = 4; fallAL = -2; }
