...
export const EXECUTION_TEMPLATES = [
  ...
  "%A decapitates %D with a swift arc of their %W",
  "As fatigue claims %D's footing, %A brings their %W crashing down with bone-shattering force",
  "A sloppy block from an exhausted %D leaves them wide open for %A's severing stroke",
  "%A drives the %W through %D's chest, ending the fight with bloody finality",
  "With %D's armor compromised, %A exploits the gap, their %W painting the sands red",
  "Lungs burning and limbs trembling, %D can only watch as %A's %W falls in a merciless arc",
  "A crushing blow from %A's %W caves in %D's desperate defense, ending their miserable existence",
  "%A turns %D's clumsy, fatigued lunge into a fatal opening, separating their head with the %W",
  "With methodical cruelty, %A dissects the ruined armor of %D, delivering the killing thrust",
  "The %W sings a morbid tune as %A expertly severs %D's vital arteries",
  "%D stumbles over their own exhausted feet, practically gifting %A the perfect execution stroke",
  "A deafening crunch echoes through the arena as %A's blunt %W completely crushes %D's skull",
  "%A mercilessly hammers down upon the staggering %D, turning flesh and bone into pulp",
  "Exploiting the ragged breathing of %D, %A lunges, sliding their %W through a massive defensive gap",
  "With surgical precision, %A carves apart %D's exposed vitals, leaving a lifeless husk",
  "A sickening snap signals the end as %A's brutal swing snaps %D's spine in twain",
  "The crowd roars as %A eviscerates the completely exhausted %D with a savage upward slash",
  "%A crushes the life from %D with a suffocating, unrelenting flurry of devastating %W blows",
  "%D's sluggish parry is batted aside like a toy, paving the way for %A's brutal, life-ending thrust",
  "In a terrifying display of raw power, %A cleaves %D nearly in half with a single, monstrous swing of the %W"
];
