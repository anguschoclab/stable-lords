...
describe("NewsletterFeed", () => {
  ...
  it("should calculate correct style rollups", () => {
    // A wins vs D
    NewsletterFeed.appendFightResult(createMockCard({
      styleA: "Brawler" as any, styleD: "Speed" as any, winner: "A", by: "KO"
    }));
    // D wins vs A
    NewsletterFeed.appendFightResult(createMockCard({
      styleA: "Speed" as any, styleD: "Power" as any, winner: "D", by: "Kill"
    }));
  ...
  it("should compute top movers based on fame + pop delta", () => {
    NewsletterFeed.appendFightResult(createMockCard({
      a: "Fighter 1", d: "Fighter 2",
      fameDeltaA: 10, popularityDeltaA: 5,
      fameDeltaD: -5, popularityDeltaD: -2
    }));
    NewsletterFeed.appendFightResult(createMockCard({
      a: "Fighter 3", d: "Fighter 1",
      fameDeltaA: 20, popularityDeltaA: 10,
      fameDeltaD: 5, popularityDeltaD: 2  // Fighter 1 gets another +7
    }));
  ...
  it("should score fights correctly for Fight of the Week", () => {
    NewsletterFeed.appendFightResult(createMockCard({
      id: "boring-fight", by: "Decision" as any
    })); // 0 points

    NewsletterFeed.appendFightResult(createMockCard({
      id: "draw-fight", by: "Draw" as any
    })); // 1 point

    NewsletterFeed.appendFightResult(createMockCard({
      id: "flashy-ko-fight", by: "KO", flashyTags: ["Flashy"]
    })); // 2 (KO) + 2 (Flashy) = 4 points

    NewsletterFeed.appendFightResult(createMockCard({
      id: "epic-comeback-kill", by: "Kill", flashyTags: ["Comeback", "Flashy"]
