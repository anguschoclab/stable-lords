...
import { pickRivalOpponent, generateRivalryNarrative, calculateRivalryScore } from "@/engine/rivals/rivalUtils";
import type { RivalStableData, Warrior } from "@/types/state.types";

describe("rivalUtils", () => {

  const createMockWarrior = (id: string, status: "Active" | "Dead" | "Injured" = "Active"): Warrior => ({
    id,
    status,
    name: `Warrior ${id}`
  } as Warrior);
  ...
  describe("pickRivalOpponent", () => {
    ...
    it("should return null if all rivals are excluded", () => {
      const rivals = [
        createMockStable("S1", [createMockWarrior("W1")])
      ];
    ...
    it("should return null if no active warriors exist", () => {
      const rivals = [
        createMockStable("S1", [createMockWarrior("W1", "Dead"), createMockWarrior("W2", "Injured")])
      ];
    ...
    it("should pick an active warrior not in the exclude list", () => {
      ...
      const rivals = [
        createMockStable("S1", [w1]),
        createMockStable("S2", [w2])
      ];
