...
describe("ExpansionService", () => {
  ...
  describe("processExpansion", () => {
    it("should process expansion for rival stables", () => {
      const { updatedState, newStables } = ExpansionService.processExpansion(state, 12345, 8);
    ...
    it("should not add stables when at target count", () => {
      const { updatedState, newStables } = ExpansionService.processExpansion(state, 12345, 5);
    ...
    it("should not add stables when above target count", () => {
      const { updatedState, newStables } = ExpansionService.processExpansion(state, 12345, 3);
    ...
    it("should integrate legacy founders when provided", () => {
      ...
      const { updatedState, newStables } = ExpansionService.processExpansion(state, 12345, 8, legacyCandidates);
    ...
    it("should set personality to Aggressive for legacy founders", () => {
      ...
      const { newStables } = ExpansionService.processExpansion(state, 12345, 8, legacyCandidates);
    ...
    it("should generate new stables with valid structure", () => {
      const { newStables } = ExpansionService.processExpansion(state, 12345, 8);
    ...
    it("should be deterministic with same seed", () => {
      const { newStables: stables1 } = ExpansionService.processExpansion(state, 12345, 8);
      const { newStables: stables2 } = ExpansionService.processExpansion(state, 12345, 8);
    ...
    it("should not exceed target count", () => {
      const { updatedState } = ExpansionService.processExpansion(state, 12345, 10);
