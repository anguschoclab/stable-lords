  ...
  describe("computeHealthImpact", () => {
    it("should process injuries and return updates", () => {
      const mockState = {
        week: 5,
        roster: [
          { id: "w1", name: "Warrior 1", injuries: [{ type: "cut", weeksRemaining: 2 }] },
      ...
      vi.spyOn(injuriesModule, "tickInjuries").mockReturnValue({
        active: [{ type: "cut", weeksRemaining: 1 }],
        healed: ["sprain"],
      });
      ...
      expect(injuriesModule.tickInjuries).toHaveBeenCalledWith([{ type: "cut", weeksRemaining: 2 }]);

      expect(impact.rosterUpdates.size).toBe(1);
      expect(impact.rosterUpdates.get("w1")).toEqual({ injuries: [{ type: "cut", weeksRemaining: 1 }] });
  ...
  describe("applyHealthUpdates", () => {
    it("should apply computed impacts to the state", () => {
      const mockState = {
        ...
        restStates: [{ warriorId: "w1", untilWeek: 4 }],
        newsletter: [],
      } as unknown as GameState;
      ...
      expect(newState.roster.find(w => w.id === "w1")?.injuries).toEqual([{ type: "cut", weeksRemaining: 1 }]);
