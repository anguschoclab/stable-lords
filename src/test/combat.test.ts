...
describe('simulateFight — kill logic', () => {
  it('kills are more likely with high killDesire and OE', () => {
    ...
    const wA = makeWarrior('Killer', FightingStyle.BashingAttack, {
      ST: 20,
      CN: 14,
      WL: 15,
      WT: 12,
    });
    const wD = makeWarrior('Victim', FightingStyle.StrikingAttack, {
      ST: 8,
      CN: 10,
      WL: 8,
      DF: 8,
...
describe('simulateFight — endurance and exhaustion', () => {
  it('high OE+AL drains endurance faster', () => {
    ...
    const aggressiveEndedByEndurance =
      rAggressive.by === 'Exhaustion' || rAggressive.by === 'Draw' || rAggressive.winner === null;
