...
import { Crown, Star, Shield, Flame, Trophy, Users, Skull, Zap, Crosshair, Activity } from "lucide-react";
...
export default function HallOfFame() {
  const { 
    roster, graveyard, retired, rivals, awards, year, player, season
  } = useGameStore();
    ...
    <div className="space-y-8">
        ...
        actions={
          <div className="flex items-center gap-4">
            <div className="flex flex-col items-end">
              <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/40">Fallen</span>
              <span className="text-xl font-mono font-black text-destructive">{graveyard.length}</span>
            </div>
            <div className="w-px h-8 bg-border/20" />
            <div className="flex flex-col items-end">
              <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/40">My Fallen</span>
      ...
      <Tabs defaultValue="halloffame" className="w-full">
        <TabsList className="bg-secondary/20 p-1 rounded-none h-12 w-full sm:w-auto">
          <TabsTrigger value="halloffame" className="flex-1 rounded-none gap-2 font-black uppercase text-[10px] tracking-widest data-[state=active]:bg-primary data-[state=active]:text-white transition-all">
            <Crown className="h-3.5 w-3.5" /> Hall of Fame
          </TabsTrigger>
          <TabsTrigger value="graveyard" className="flex-1 rounded-none gap-2 font-black uppercase text-[10px] tracking-widest data-[state=active]:bg-destructive data-[state=active]:text-white transition-all">
                              ...
                              <div className="flex items-start justify-between">
                                <div className="space-y-1">
                                  ...
                                  <div className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60">
                                    Leader: {player.stableName === award.stableName ? player.name : "Rival Owner"}
                                  </div>
                                </div>
        ...
        <TabsContent value="graveyard" className="mt-8">
          <Tabs defaultValue="memorial" className="w-full">
            ...
            <TabsContent value="world">
              <FallenGrid warriors={graveyard} season={season} emptyTitle="Sands of Peace" emptyDesc="No blood has been spilled in this realm." />
            </TabsContent>
          </Tabs>
        </TabsContent>
      </Tabs>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {warriors.map((w, idx) => (
        ...
        >
          <Surface variant="glass" padding="none" className="group relative border-border/40 overflow-hidden hover:border-destructive/40 transition-all duration-500 shadow-2xl h-full flex flex-col">
            ...
            <div className="p-8 flex-1 flex flex-col">
              <div className="flex justify-between items-start mb-8">
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  {(w.career?.medals?.gold ?? 0) > 0 && <Badge className="bg-arena-gold text-black text-[8px] font-black tracking-widest px-2">Champion</Badge>}
                </div>
              </div>
              ...
              <Surface variant="blood" className="p-5 mb-8 relative border-destructive/10">
                ...
                <div className="space-y-3 border-t border-destructive/10 pt-4 mt-auto">
                  ...
                  <div className="flex items-center justify-between text-[10px] font-mono font-black text-muted-foreground/30">
                    <span>Week {w.deathWeek || '??'} · {season?.toUpperCase()}</span>
                    <span className="flex items-center gap-1.5"><Activity className="h-3 w-3" /> Remains recovered</span>
                  </div>
                </div>
              </Surface>
