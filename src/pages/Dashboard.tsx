...
const WIDGET_REGISTRY: WidgetDef[] = [
  ...
  { id: 'rivals', label: 'RIVAL STABLES', component: MemoRivalsWidget },
  {
    ...
    component: MemoStableComparisonWidget,
    wide: true,
  },
  ...
  return (
    <div className="space-y-12 max-w-7xl mx-auto">
      <PageHeader
        title={
          <EditableText
            value={player.stableName}
            onSave={renameStable}
            className="text-4xl font-black tracking-tighter text-foreground sm:text-6xl font-display uppercase"
          />
        }
        subtitle={
          ...
          <div className="flex flex-col md:flex-row items-center gap-4">
            <div className="flex items-center gap-4 text-sm bg-neutral-900/40 backdrop-blur-md px-4 py-2 rounded-none border border-white/5 shrink-0 shadow-inner">
              <div className="flex items-center gap-2 border-r border-white/5 pr-4">
                <span className="text-muted-foreground text-[9px] uppercase tracking-widest font-black opacity-60">
                  GOLD
                </span>
                <span className="font-mono text-arena-gold font-black">
                  {Math.round(treasury).toLocaleString()}G
                </span>
              </div>
              <div className="flex items-center gap-2 border-r border-white/5 pr-4">
                <span className="text-muted-foreground text-[9px] uppercase tracking-widest font-black opacity-60">
                ...
                <span className="font-mono text-arena-fame font-black">
                  {Math.round(fame).toLocaleString()}
                </span>
              </div>
            </div>
          ...
          return (
            <div
              ...
              className={cn(
                ...
                isDragOver &&
                  'ring-2 ring-primary/50 ring-offset-1 ring-offset-background rounded-none'
              )}
            >
              {isEditing && (
