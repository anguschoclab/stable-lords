...
function calculatePromoterStats(promoterId: string, offers: Record<string, BoutOffer>, currentWeek: number) {
  const promoterOffers = Object.values(offers).filter(o => o.promoterId === promoterId);
  const activeThisWeek = promoterOffers.filter(o => o.boutWeek === currentWeek && o.status === "Signed").length;
      ...
      <CardContent className="space-y-4 pt-0">
        ...
        <div className="space-y-1.5">
          <div className="flex justify-between text-[10px] uppercase tracking-wider">
            ...
            <span className={`font-mono font-bold ${capacityPercent >= 80 ? "text-red-500" : capacityPercent >= 50 ? "text-amber-500" : "text-emerald-500"}`}>
...
export default function PromoterDirectory() {
  ...
  const { sortedPromoters, stats } = useMemo(() => {
    ...
    const totalNotableBouts = promoterList.reduce((sum, p) => sum + (p.history?.notableBouts?.length || 0), 0);
    const totalCapacity = promoterList.reduce((sum, p) => sum + p.capacity, 0);
    const totalActiveOffers = Object.values(boutOffers || {}).filter(o => o.status === "Signed").length;
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
        ...
        <Card className="bg-gradient-to-br from-emerald-500/5 to-transparent">
          <CardContent className="p-4 space-y-1">
            ...
            <div className="text-2xl font-black font-mono">{stats.totalActiveOffers}/{stats.totalCapacity}</div>
          </CardContent>
        </Card>
      </div>
