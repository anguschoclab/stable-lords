...
function TournamentSection({ award }: { award: TournamentAward }) {
  ...
  return (
    <article className="space-y-4">
              ...
              {award.upsets.map((u, i) => (
                <div key={i} className="flex items-center justify-between py-1.5 px-3 rounded-lg bg-secondary/40 border border-border/30">
                  ...
                  <div className="flex items-center gap-2 text-[10px] font-mono">
                    ...
                    <span className="text-muted-foreground">Rd {u.round}</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* Fight List */}
      {award.fights.length > 0 && (
        <Collapsible>
          <CollapsibleTrigger className="flex items-center gap-2 text-sm font-display font-semibold text-muted-foreground hover:text-foreground transition-colors group w-full">
            <Swords className="h-4 w-4 text-muted-foreground" />
            All Bouts ({award.fights.length})
            <ChevronDown className="h-3 w-3 transition-transform group-data-[state=open]:rotate-180 ml-auto" />
          </CollapsibleTrigger>
          <CollapsibleContent>
            <div className="mt-2 space-y-1">
              {award.fights.map((f) => {
                const bout = award.tournament.bracket.find(b => b.fightId === f.id);
                return (
                  <div key={f.id} className="flex items-center justify-between py-1.5 px-3 rounded-lg bg-secondary/30 border border-border/20 text-xs">
                    <div className="flex items-center gap-2">
                      {bout && <span className="text-[10px] font-mono text-muted-foreground w-8">Rd {bout.round}</span>}
                      <span className={cn("font-display", f.winner === "A" ? "font-bold text-foreground" : "text-muted-foreground")}>{f.a}</span>
                      <span className="text-muted-foreground">vs</span>
                      <span className={cn("font-display", f.winner === "D" ? "font-bold text-foreground" : "text-muted-foreground")}>{f.d}</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      {f.by && (
                        <Badge variant={f.by === "Kill" ? "destructive" : "outline"} className="text-[8px]">
