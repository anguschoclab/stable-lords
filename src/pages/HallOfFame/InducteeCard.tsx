...
export function InducteeCard({ warrior, title, icon, fights }: { warrior: Warrior; title: string; icon: React.ReactNode; fights: FightSummary[] }) {
  ...
  return (
    ...
    >
      <Card className="bg-glass-card border border-border/40 hover:border-arena-gold/40 transition-all duration-500 overflow-hidden relative group">
        ...
        <CardContent className="p-6 space-y-5">
          ...
          <div className="flex items-start justify-between">
            ...
            <Badge variant="outline" className="text-[9px] font-black tracking-widest uppercase text-arena-gold bg-arena-gold/5 border-arena-gold/20 py-1 px-2">
              {title}
            </Badge>
          </div>
          ...
          <div className="space-y-2 py-2">
            <div className="flex items-center justify-between text-[10px] font-black tracking-[0.2em] text-muted-foreground/60">
              <span className="flex items-center gap-1.5"><Activity className="h-3 w-3" /> PERFORMANCE_RATIO</span>
            ...
            <div className="bg-black/20 rounded-xl p-4 border border-border/10 mt-auto">
              ...
              <div className="flex items-center justify-between">
                <span className={cn(
                  ...
                  best.winner === "A" && best.a === warrior.name || best.winner === "D" && best.d === warrior.name ? "text-foreground" : "text-muted-foreground/40"
                )}>
                  {best.a}
                </span>
                <div className="flex flex-col items-center gap-1">
                  <Swords className="h-3 w-3 text-muted-foreground/40" />
                  <Badge variant="outline" className="text-[8px] font-black py-0 px-1 border-muted-foreground/20">{best.by}</Badge>
                </div>
                <span className={cn(
                  "text-[11px] font-display font-black uppercase tracking-tight text-right",
                  best.winner === "D" && best.d === warrior.name || best.winner === "A" && best.a === warrior.name ? "text-foreground" : "text-muted-foreground/40"
                )}>
                  {best.d}
                </span>
              </div>
              {best.flashyTags && best.flashyTags.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mt-3 justify-center">
                  {best.flashyTags.map(t => (
                    <span key={t} className="text-[8px] font-black uppercase tracking-widest text-arena-gold/60 border border-arena-gold/10 bg-arena-gold/5 px-2 py-0.5 rounded-full">{t}</span>
                  ))}
                </div>
              )}
            </div>
