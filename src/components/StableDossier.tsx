...
export function StableDossier({ stableId, stableName }: StableDossierProps) {
  ...
  const stable = useMemo(() => {
    ...
    const rival = state.rivals.find(r => r.owner.id === stableId || r.owner.stableName === stableName);
    if (rival) return { ...rival, isPlayer: false };
    return undefined;
  }, [stableId, stableName, state.player, state.roster, state.rivals]);

  if (!stable) return <div className="p-8 text-center text-muted-foreground">Stable not found.</div>;
        ...
        <div className="grid grid-cols-2 gap-3">
          <Card className="bg-secondary/20 border-none">
            <CardContent className="p-3 text-center">
              ...
              <div className="text-xl font-display font-black text-arena-fame">{stable.owner.fame}</div>
            </CardContent>
          </Card>
          <Card className="bg-secondary/20 border-none">
            <CardContent className="p-3 text-center">
              <div className="text-[10px] uppercase font-bold text-muted-foreground mb-1">Roster</div>
              <div className="text-xl font-display font-black text-primary">{stable.roster.filter(w => w.status === "Active").length}</div>
            </CardContent>
          </Card>
        </div>
