...
export function WarriorComparison({ rivals, playerRoster }: WarriorComparisonProps) {
  ...
  const allWarriors = useMemo(() => {
    ...
    for (const w of playerRoster.filter((w) => w.status === 'Active'))
      list.push({ warrior: w, stable: 'User Stable' });
    for (const r of rivals) {
      for (const w of r.roster.filter((w) => w.status === 'Active'))
        list.push({ warrior: w, stable: r.owner.stableName });
    }
    return list;
  }, [rivals, playerRoster]);
      ...
      >
        ...
        <div className="space-y-1">
          <p className="text-sm font-display font-black uppercase tracking-tight text-muted-foreground">
            Personnel Database Depleted
          </p>
          <p className="text-xs text-muted-foreground/60 italic">
            Ensure at least two active combatants are registered in the global registry.
          </p>
        </div>
      </Surface>
    );
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-2 gap-8">
        {[
          ...
          { sel: wIdB, setSel: setWIdB, other: wIdA, label: 'Asset Beta', color: 'accent' },
        ].map(({ sel, setSel, other, label, color }) => (
            ...
            <div className="grid grid-cols-1 gap-1.5 max-h-96 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
              {allWarriors.map(({ warrior: w, stable }) => (
                <Tooltip key={w.id}>
                  ...
                  <TooltipContent
                    side={label === 'Asset Alpha' ? 'left' : 'right'}
      ...
      {entryA && entryB && (
        <div className="space-y-6">
          <Surface
            variant="glass"
            padding="none"
            className="border-border/40 relative overflow-hidden flex items-center justify-between p-8"
          >
            <div className="absolute inset-0 bg-gradient-to-r from-primary/5 via-transparent to-accent/5 pointer-events-none" />

            <div className="text-center flex-1 relative z-10 space-y-3">
              <h4 className="text-[9px] font-black tracking-[0.4em] text-primary uppercase leading-none opacity-40">
                Personnel Alpha
              </h4>
              <h3 className="font-display font-black uppercase text-xl tracking-tighter text-foreground leading-none">
                {entryA.warrior.name}
              </h3>
              <div className="flex flex-col items-center gap-1">
                <span className="text-[9px] text-muted-foreground/60 font-black tracking-widest uppercase">
                  {STYLE_DISPLAY_NAMES[entryA.warrior.style as keyof typeof STYLE_DISPLAY_NAMES] ??
                    entryA.warrior.style}
                </span>
                <span className="text-[8px] text-primary font-bold uppercase tracking-tighter opacity-40">
                  {entryA.stable}
                </span>
              </div>
            </div>
            ...
            <div className="text-center flex-1 relative z-10 space-y-3">
              <h4 className="text-[9px] font-black tracking-[0.4em] text-accent uppercase leading-none opacity-40">
                Personnel Beta
              </h4>
              <h3 className="font-display font-black uppercase text-xl tracking-tighter text-foreground leading-none">
                {entryB.warrior.name}
              ...
              <div className="flex flex-col items-center gap-1">
                <span className="text-[9px] text-muted-foreground/60 font-black tracking-widest uppercase">
                  {STYLE_DISPLAY_NAMES[entryB.warrior.style as keyof typeof STYLE_DISPLAY_NAMES] ??
                    entryB.warrior.style}
                </span>
                <span className="text-[8px] text-accent font-bold uppercase tracking-tighter opacity-40">
                  {entryB.stable}
                </span>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <Surface variant="glass" className="border-border/40 space-y-6">
              ...
              <div className="space-y-4">
                <ComparisonBar
                  label="Wins"
                  valA={entryA.warrior.career.wins}
                  valB={entryB.warrior.career.wins}
                  maxVal={Math.max(entryA.warrior.career.wins, entryB.warrior.career.wins, 1)}
                  colorA="bg-primary"
                  colorB="bg-accent"
                />
                <ComparisonBar
                  label="Losses"
                  valA={entryA.warrior.career.losses}
                  valB={entryB.warrior.career.losses}
                  maxVal={Math.max(entryA.warrior.career.losses, entryB.warrior.career.losses, 1)}
                  colorA="bg-primary"
                  colorB="bg-accent"
                />
                <ComparisonBar
                  label="Fatalities"
                  valA={entryA.warrior.career.kills}
                  valB={entryB.warrior.career.kills}
                  maxVal={Math.max(entryA.warrior.career.kills, entryB.warrior.career.kills, 1)}
                  colorA="bg-primary"
                  colorB="bg-accent"
                />
                <ComparisonBar
                  label="Prestige"
                  valA={entryA.warrior.fame ?? 0}
                  valB={entryB.warrior.fame ?? 0}
                  maxVal={Math.max(entryA.warrior.fame ?? 0, entryB.warrior.fame ?? 0, 1)}
                  colorA="bg-primary"
                  colorB="bg-accent"
                />
                <ComparisonBar
                  label="Popularity"
                  valA={entryA.warrior.popularity ?? 0}
                  valB={entryB.warrior.popularity ?? 0}
                  maxVal={Math.max(
                    entryA.warrior.popularity ?? 0,
                    entryB.warrior.popularity ?? 0,
                    1
                  )}
                  colorA="bg-primary"
                  colorB="bg-accent"
