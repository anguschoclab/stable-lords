...
export function TacticalBar() {
  ...
  const { roster, trainers, trainingAssignments, boutOffers, isTournamentWeek, day, week } = useGameStore();
  ...
  const alerts = useMemo<Alert[]>(() => {
    ...
    const assignedIds = new Set(trainingAssignments?.map((a: { warriorId: string }) => a.warriorId) ?? []);
    ...
    const offersArray: BoutOffer[] = boutOffers ? Object.values(boutOffers as Record<string, BoutOffer>) : [];
    ...
    const fightReady = roster?.filter((w: unknown) => isFightReady(w as Parameters<typeof isFightReady>[0])) ?? [];
