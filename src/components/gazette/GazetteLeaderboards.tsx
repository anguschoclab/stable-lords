...
export function GazetteLeaderboard({ allFights }: LeaderboardProps) {
  ...
  return (
    <Surface variant="glass" padding="none" className="border-border/10 bg-neutral-900/40 overflow-hidden shadow-2xl relative">
      ...
      <div className="p-8 border-b border-white/5 flex items-center justify-between bg-neutral-900/60 relative z-10">
        <div className="flex items-center gap-4">
           ...
           <div>
              <h3 className="font-display text-base font-black uppercase tracking-tight">Combatant_Efficiency_Registry</h3>
              <p className="text-[9px] text-muted-foreground font-black uppercase tracking-widest opacity-40">Elite_Performance // Ranking_Sync_Ready</p>
           </div>
        </div>
        ...
        <Table>
          <TableHeader className="bg-black/20">
            <TableRow className="hover:bg-transparent border-white/5">
              <TableHead className="font-black uppercase text-[10px] tracking-widest pl-8 py-4">Operative_ID</TableHead>
              <TableHead className="font-black uppercase text-[10px] tracking-widest text-muted-foreground/60 py-4">Martial_Discipline</TableHead>
              <TableHead className="font-black uppercase text-[10px] tracking-widest text-center text-muted-foreground/60 py-4">Engagement_Record</TableHead>
              <TableHead className="font-black uppercase text-[10px] tracking-widest text-right text-muted-foreground/60 py-4">Efficiency_Index</TableHead>
              <TableHead className="font-black uppercase text-[10px] tracking-widest text-right pr-8 py-4">Fame_Manifest</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {leaderData.map((w, idx) => (
              <TableRow key={w.name} className="border-white/5 group hover:bg-white/2 transition-colors">
                <TableCell className="pl-8 py-5">
                   <div className="flex items-center gap-4">
                      <span className="text-xs font-mono font-black text-white/20">#{idx + 1}</span>
                      <span className="font-display font-black text-sm uppercase tracking-tight group-hover:text-primary transition-colors">{w.name}</span>
                   </div>
                </TableCell>
                <TableCell className="py-5">
                   <Badge variant="outline" className="text-[9px] font-black border-white/5 bg-secondary/20 text-muted-foreground/80 uppercase tracking-widest px-3">
                      {w.style}
                   </Badge>
                </TableCell>
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <div className="flex items-center justify-center gap-3 text-xs font-mono font-black">
                        ...
                        <span className="text-white/10">|</span>
                        <span className="text-arena-gold drop-shadow-[0_0_5px_rgba(255,215,0,0.3)]">{w.k}K</span>
                      </div>
                    </TooltipTrigger>
                ...
                <TableCell className="text-right py-5">
                  <span className="text-xs font-mono font-black text-primary/80 group-hover:text-primary transition-colors">{(w.rate * 100).toFixed(1)}%</span>
                </TableCell>
  ...
  return (
    <Surface variant="glass" className="border-border/10 bg-neutral-900/40 relative overflow-hidden h-full">
      ...
      <div className="flex items-center gap-4 mb-8 relative z-10">
         ...
         <div>
            <h3 className="font-display text-base font-black uppercase tracking-tight">Discipline_Vanguard</h3>
            <p className="text-[9px] text-muted-foreground font-black uppercase tracking-widest opacity-40">Style_Excellence // Master_Registry</p>
         </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 relative z-10">
        {bestData.map(d => (
          <Surface key={d.style} variant="paper" padding="sm" className="bg-white/[0.02] border-white/5 group hover:border-arena-gold/30 transition-all">
             <div className="flex justify-between items-center">
                <div className="flex flex-col">
                   <span className="text-[9px] font-black uppercase tracking-[0.2em] text-muted-foreground/60 mb-1">{d.style}</span>
                   <span className="text-xs font-display font-black uppercase text-foreground group-hover:text-arena-gold transition-colors">{d.name}</span>
                </div>
  ...
  return (
    <Surface variant="glass" className="border-border/10 bg-neutral-900/40 relative overflow-hidden h-full">
      ...
      <div className="flex items-center gap-4 mb-8 relative z-10">
         ...
         <div>
            <h3 className="font-display text-base font-black uppercase tracking-tight">Emerging_Operatives</h3>
            <p className="text-[9px] text-muted-foreground font-black uppercase tracking-widest opacity-40">Rising_Potential // Talent_Scan</p>
         </div>
      </div>
      ...
      <div className="space-y-4 relative z-10">
        {stars.length === 0 ? (
          <div className="py-12 text-center opacity-20 italic">
            <span className="text-[10px] uppercase tracking-widest">No breakthrough signals detected...</span>
          </div>
        ) : (
          stars.map(s => (
            <Surface key={s.name} variant="paper" padding="sm" className="bg-primary/5 border border-primary/10 group hover:border-primary/40 transition-all">
               <div className="flex justify-between items-center">
                  <div className="flex flex-col">
                     <span className="text-xs font-display font-black uppercase text-foreground group-hover:text-primary transition-colors">{s.name}</span>
                     <span className="text-[8px] font-black text-muted-foreground uppercase tracking-widest mt-0.5">ESTABLISHED: WK_{s.firstWeek}</span>
                  </div>
                  <div className="flex flex-col items-end">
                     <div className="flex items-center gap-2">
                        <Badge variant="outline" className="text-[9px] font-mono font-black border-primary/20 bg-primary/10 text-primary">
