...
export function RankingsWidget() {
  ...
  const ranked = useMemo(
    () => [...selectActiveWarriors(state)].sort((a, b) => b.fame - a.fame).slice(0, 5),
    [state]
  );
      ...
      <div className="px-6 py-3 bg-primary/5 border-b border-primary/10 relative z-10 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
          {playerStanding.bestWarriorName && playerStanding.bestRank !== null ? (
            <>
              <span className="text-[10px] font-mono font-black text-primary truncate w-full max-w-[120px]">
                {playerStanding.bestWarriorName}
              </span>
              <Badge
                variant="outline"
                className="text-[9px] font-mono font-black border-primary/20 bg-primary/10 text-primary h-5 px-2 tracking-widest"
              >
                #{playerStanding.bestRank}
              </Badge>
              <Badge
                variant="outline"
                className="text-[9px] font-mono font-black border-white/10 bg-white/5 text-muted-foreground/60 h-5 px-2 tracking-widest"
              >
              ...
              >
                ...
                <div className="flex items-center gap-4 shrink-0">
                  <span
                    className={cn(
                      ...
                      i === 0
                        ? 'text-arena-gold drop-shadow-[0_0_8px_rgba(255,215,0,0.4)]'
                        : i === 1
                          ? 'text-primary/60'
                  ...
                  <div
                    className={cn(
                      ...
                      i === 0
                        ? 'bg-arena-gold shadow-[0_0_15px_rgba(255,215,0,0.5)] scale-y-110'
                        : i === 1
                          ? 'bg-primary/40'
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3">
                    ...
                    <Badge
                      variant="outline"
                      className="text-[8px] font-black border-white/5 bg-secondary/20 text-muted-foreground/80 uppercase tracking-widest px-2"
                    >
                      {w.style}
                    </Badge>
                  </div>
