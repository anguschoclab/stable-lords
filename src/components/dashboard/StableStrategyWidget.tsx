...
export function StableStrategyWidget() {
  ...
  const activeGrudges = ownerGrudges.filter(
    (g) => g.ownerIdA === player.id || g.ownerIdB === player.id
  );
      ...
      <div className="space-y-3">
            ...
            <div className="space-y-1 max-h-24 overflow-y-auto">
              {activeGrudges.map((grudge) => {
                ...
                return (
                  <div
                    key={grudge.id}
                    className="flex items-center justify-between text-[9px] bg-neutral-900/40 px-2 py-1 rounded"
                  >
                    <span className="text-muted-foreground">
                      {rival?.owner.stableName || 'Unknown'}
                    </span>
        ...
        <div className="pt-2 border-t border-white/5">
          <p className="text-[9px] text-muted-foreground leading-tight">
            Your philosophy influences AI decision-making. Grudges increase rivalry intensity in
            matchups.
          </p>
        </div>
      </div>
