    ...
    <Surface variant="glass" padding="none" className="overflow-hidden border-border/10">
      ...
      <div className="p-5 space-y-6">
        {TOURNAMENT_TIERS.map((tier) => {
          ...
          const playerWarriorsInZone = roster
            .map((w) => ({
              w,
          ...
          return (
            <div key={tier.id} className="space-y-4">
              <div className="flex justify-between items-center px-1">
                ...
                <Badge
                  variant="outline"
                  className="text-[8px] font-black uppercase tracking-widest text-muted-foreground/40 border-white/5 bg-white/[0.02]"
                >
                  Cut-off: #{cutOff}
                </Badge>
              </div>
              ...
              <div className="space-y-3">
                {playerWarriorsInZone.length === 0 ? (
                  <p className="px-1 text-[9px] uppercase font-black tracking-widest text-muted-foreground/20 italic">
                    NO CONTENDERS IN ZONE
                  </p>
                ) : (
                  playerWarriorsInZone.map(({ w, rank }) => {
                    const isSafe = rank <= cutOff;
                    const distToCutoff = rank - cutOff;
                    const intensity = isSafe
                      ? Math.max(0, 100 - (rank / cutOff) * 100)
                      : Math.max(0, 100 - (distToCutoff / 20) * 100);

                    return (
                      <div
                        key={w.id}
                        className={cn(
                          'p-2 border border-white/5 bg-black/20 flex items-center gap-3 transition-colors hover:bg-black/30',
                          isSafe
                            ? 'border-l-2 border-l-arena-gold'
                            : 'border-l-2 border-l-destructive/40'
                        )}
                      >
                        <div className="flex-1 space-y-2">
                          <div className="flex justify-between items-center text-[9px] font-black uppercase tracking-widest">
                            <span className={isSafe ? 'text-arena-gold' : 'text-destructive/80'}>
                              {w.name}
                            </span>
