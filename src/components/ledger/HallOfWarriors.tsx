    ...
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <section className="space-y-6">
        <div className="flex items-center gap-4 px-1">
           ...
           <div>
              <h3 className="font-display text-lg font-black uppercase tracking-tight text-arena-gold">Legends_Registry_Eminent</h3>
              <p className="text-[9px] text-muted-foreground font-black uppercase tracking-widest opacity-40">Immortal_Status // Assets_Recovered: {retired.length}</p>
           </div>
          ...
          <div className="overflow-x-auto custom-scrollbar">
            {retired.length === 0 ? (
              <div className="py-24 text-center opacity-30 group">
                ...
                <p className="text-sm font-display font-black uppercase tracking-[0.3em]">Registry_Currently_Vacant</p>
              ...
              <Table>
                <TableHeader className="bg-arena-gold/10 sticky top-0 z-10 backdrop-blur-md">
                  <TableRow className="hover:bg-transparent border-arena-gold/10">
                    ...
                    <TableHead className="font-black uppercase text-[10px] tracking-widest text-center text-arena-gold/50 py-4">Imperial_Medals</TableHead>
                    <TableHead className="font-black uppercase text-[10px] tracking-widest text-center text-arena-gold/50 py-4">Final_Record</TableHead>
                    ...
                    <TableRow key={w.id} className="border-arena-gold/5 hover:bg-arena-gold/5 transition-all group">
                      <TableCell className="pl-8 py-5">
                         <div className="flex flex-col">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                               ...
                               <span className="text-[8px] font-black text-arena-gold/40 uppercase tracking-widest">RANK: EMERITUS</span>
                            </div>
                         </div>
                      </TableCell>
                            ...
                            {w.career?.medals && (
                              <>
                                {w.career.medals.gold ? (
                                  <span className="p-1 rounded bg-arena-gold/20 text-arena-gold text-[10px] font-black">🥇{w.career.medals.gold}</span>
                                ) : null}
                                {w.career.medals.silver ? (
                                  <span className="p-1 rounded bg-slate-400/20 text-slate-400 text-[10px] font-black">🥈{w.career.medals.silver}</span>
                                ) : null}
                                {w.career.medals.bronze ? (
                                  <span className="p-1 rounded bg-orange-600/20 text-orange-600 text-[10px] font-black">🥉{w.career.medals.bronze}</span>
                                ) : null}
                              </>
                            )}
                         </div>
                      ...
                      <TableCell className="text-right py-5">
                         <div className="flex flex-col items-end">
                            ...
                            <span className="text-[8px] font-black text-arena-gold/30 uppercase tracking-widest mt-0.5">LEGACY_FAME</span>
                         </div>
                      </TableCell>
