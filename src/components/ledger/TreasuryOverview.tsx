  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "Treasury Reserve", value: `${gold.toLocaleString()}G`, icon: Wallet, color: "text-arena-gold", variant: "gold" as const, desc: "Liquid capital available for operations and recruitment." },
          { label: "Active Personnel", value: activeWarriors.length, icon: Swords, color: "text-primary", variant: "glass" as const, desc: "Total combatant assets currently on active deployment." },
          { label: "Total Wins", value: totalWins, icon: TrendingUp, color: "text-arena-pop", variant: "glass" as const, desc: "Cumulative competitive victories across all career rosters." },
          { label: "Arena Kills", value: totalKills, icon: Skull, color: "text-destructive", variant: "blood" as const, desc: "Final cessation incidents recorded during arena engagements." },
        ].map((stat, i) => (
          <Tooltip key={i}>
        ...
        <Surface variant="glass" className="lg:col-span-4 border-primary/10 relative overflow-hidden flex flex-col">
          ...
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-display text-sm font-black uppercase tracking-tight">Fiscal Trajectory</h3>
                <p className="text-[9px] text-muted-foreground font-black uppercase tracking-widest opacity-40">Operating Liquidity · WK {state.week.toString().padStart(2, '0')}</p>
              </div>
            </div>
            {breakdown.net >= 0 ? (
          ...
          <div className="space-y-8 flex-1">
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                 ...
                 <span className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground">Revenue Streams</span>
              ...
              <div className="space-y-2">
                ...
                {breakdown.income.length === 0 && (
                   <p className="text-[10px] text-muted-foreground/30 italic uppercase tracking-widest py-2 text-center">No active revenue</p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                 ...
                 <span className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground">Operational Costs</span>
                 <div className="h-px flex-1 bg-white/5" />
              </div>
              <div className="space-y-2">
                ...
                {breakdown.expenses.length === 0 && (
                   <p className="text-[10px] text-muted-foreground/30 italic uppercase tracking-widest py-2 text-center">No active debits</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="mt-8 pt-6 border-t border-white/5">
             <Surface variant="paper" padding="sm" className="bg-black/40 border border-white/5 flex justify-between items-center group hover:border-primary/30 transition-all">
                <div className="flex flex-col">
                   <span className="text-[8px] font-bold text-muted-foreground uppercase tracking-[0.3em] opacity-40">Weekly Net Flux</span>
                   <span className="text-xs font-black uppercase tracking-widest text-foreground group-hover:text-primary transition-colors">Projected Net</span>
                </div>
                <div className={cn(
        ...
        <Surface variant="glass" padding="none" className="lg:col-span-8 border-border/10 flex flex-col relative overflow-hidden h-[500px]">
          <div className="p-8 border-b border-white/5 bg-neutral-900/40 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                 <h3 className="font-display text-base font-black uppercase tracking-tight">Ledger Registry</h3>
                 <p className="text-[9px] text-muted-foreground font-black uppercase tracking-widest opacity-40">Audited Transaction History · {state.ledger?.length ?? 0} entries</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
               <div className="h-1 w-1 rounded-full bg-arena-gold animate-pulse" />
               <span className="text-[8px] font-black uppercase tracking-widest text-arena-gold opacity-60">Verified by Scribes</span>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-auto custom-scrollbar">
            {recentLedger.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center p-12 text-center opacity-20 group">
                ...
                <p className="text-sm font-display font-black uppercase tracking-[0.3em]">Registry Empty</p>
                <p className="text-[10px] lowercase italic opacity-80 mt-2 font-medium">Bout engagement required for transaction logging...</p>
              </div>
            ) : (
              <Table>
                <TableHeader className="bg-black/20 sticky top-0 z-10 backdrop-blur-md border-b border-white/5">
                  <TableRow className="hover:bg-transparent border-white/5">
                    ...
                    <TableHead className="font-black uppercase text-[10px] tracking-widest text-muted-foreground/60">DESCRIPTION</TableHead>
          ...
          <div className="p-4 border-t border-white/5 bg-black/40 flex justify-center">
             <button aria-label="Access Full Archive" className="text-[9px] font-black uppercase tracking-[0.4em] text-muted-foreground hover:text-primary transition-colors opacity-40 hover:opacity-100 flex items-center gap-2 group">
                Access Full Archive <ArrowDownRight className="h-3 w-3 group-hover:translate-y-0.5 transition-transform" />
             </button>
          </div>
        </Surface>
      </div>
